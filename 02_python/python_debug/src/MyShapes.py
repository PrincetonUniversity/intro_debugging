...
class Trapezoid(object):
  def __init__(self, par_side_1, par_side_2, height):
    ...
    self.height = height
  def area(self):
    sum_par_side = self.par_side_1 + self.par_side_2
    return 0.5 * sum_par_side * sum_par_side
...
class RightCircularCone(object):
  ...
  def surf_area(self):
    ...
    body_area = math.sqrt(self.radius * self.radius * self.height * self.height)
    return  body_area + base_area
  def volume(self):
    return  0.3333333 * math.pi * self.radius * self.radius * self.height
